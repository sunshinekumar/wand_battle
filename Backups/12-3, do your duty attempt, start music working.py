...
class GameRuntime(object): 
    def __init__(self): 
        ...
        self.initiate_circle_text = """Initiate"""
        self.circle_text_size = 90

        #Start circles
        (self.start, self.practice) = (0,1)

        # Defining spells
        (self.north,self.south, self.east, self.west, self.clear) = (0,1,2,3,4)

        #They are easier to reference as variable names
        #Sounds from https://www.youtube.com/watch?v=29fvdnzZ0cw
        # and http://soundfxcenter.com/sound_effect/search.php?sfx=Harry+Potter
        # and https://www.youtube.com/watch?v=1MK78oTKFGM
        self.expelliarmus = "Expelliarmus"
        self.expelliarmus_power = 20
        self.expelliarmus_sound = ""
    ...
    def draw_circles(self, joint_points):
        ...
        self.circle_text_size =  abs(int(joint_points[PyKinectV2.JointType_Head].y - joint_points[PyKinectV2.JointType_SpineShoulder].y))//2
    ...
    def draw_circles2(self, joint_points):
        ...
        self.draw_text(self.initiate_circle_text, self.circles2[-1], self.spell_circle_color, self.initiate_circle_text_size)
    ...
    def draw_start_circles2(self, joint_points):
        ...
        #This is the code for drawing all circles text
        self.circle_text_size =  abs(int(joint_points[PyKinectV2.JointType_Head].y - joint_points[PyKinectV2.JointType_SpineShoulder].y))//2
        text = list(map(lambda x: str(x), self.circles))
        for i in range(len(self.circles)):
            self.draw_text(text[i], (self.circles[i][0], self.circles[i][1] - self.circle_radius*2), self.wand_color, self.circle_text_size)
            '''
    ...
    def run(self): 
        ...
        while not self._done: 
            ...
            if self.spell != "" and self.do_your_duty_end_flag == 1:
            ...
            if self.spell2 != "" and self.do_your_duty_end_flag == 1:
