...
#Title Screen image from https://www.pottermore.com/writing-by-jk-rowling/mr-ollivander
...
class GameRuntime(object): 
    def __init__(self): 
        ...
        self.circle_text_size = 120
        self.circle_text_size2 = 120

        #Customize circles
        (self.red, self.green, self.blue, self.length) = (0,1,2,3)
        ...
        self.avada_kedavra = "Avada Kedavra"
        ...
        self.avada_kedavra_failed = 'Sound Files/failed avada kedavra.mp3'
        ...
        self.spell_book = {(self.south, self.north): self.expelliarmus, 
                           ...
                           (self.north, self.south, self.east, self.west): self.avada_kedavra}
        
        self.tutorial_text = "Expelliarmus (south, north) \nStupefy (east, west) \nProtego (west, north, east) \nAvada Kedavra (north, south, east, west)"
        ...
        #Intro Screen
        self.title_screen = 1
        self.title_image_path = 'Title Screen.jpg'
        self.title_text = "DUEL"
        self.title_text_size = 200
        self.title_color = (255,255,255)
        self.title_image = pygame.image.load(self.title_image_path)
        self.title_screen_sound_flag = 0
        self.title_font = 'yerbaluisa.ffp.ttf'
        ...
        self.bow = 'Sound Files/First we bow.mp3'
        pygame.mixer.music.load(self.bow)
        ...
        self.customize_tracker = -1
        self.customize_tracker2 = -1
        self.max_wand_scale = 2
        self.min_wand_scale = .3
        self.wand_scale_change_speed = .1
        self.wand_color_change_speed = 15

        #Main Tutorial
        self.main_tutorial = 0
        self.main_tutorial_text_size = 40
        self.main_tutorial_text = """WELCOME TO THE SPELLCASTING 101
        \nAbove you are your health bar (green) and power bar (black)
        \n  if your health is low, the bar will turn red
        \n  if you are not at full power, the bar will turn gray
        \nSPELLS:
        \nexpelliarmus disarms opponent and uses 20 power
        \nStupefy attacks opponent for 30 damage and uses 30 power
        \nProtego casts a shield that blocks the next attack - Uses 50 power
        \nAvada Kedavra, The Killing Curse, has a small chance of killing the opponent, high chance of cursing you
        \nLeft Hand:
        \n  Swiping right restores power
        \n  Swiping up brings up the mini tutorial
        \n  Swiping down closes the mini tutorial
        \nPractice mode is a one player mode where a backfired spell cannot damage you
        \nIn Customize mode, drop your wand on a token and swipe right with the left hand to increase the value of the token you are on, left to decrease
        \n  If the wand is not in a token, swiping will change the size
        \nFrom anywhere, put your hands together to pause your game and return to the start screen
        \nonce a superior wizard emerges, bow to restart the game
        """
    ...
    def draw_tutorials(self):
        ...
        if(self.tutorial_up == 1):
            pygame.draw.rect(self._frame_surface, (255-self.wand_color[0],255-self.wand_color[1],255-self.wand_color[2]), 
                             (0,0,self.screen_width//2, self.tutorial_text_size * self.tutorial_text.count("\n")))
        ...
        if(self.tutorial_up2 == 1):
            pygame.draw.rect(self._frame_surface, (255-self.wand_color2[0],255-self.wand_color2[1],255-self.wand_color2[2]), 
                             (self.screen_width//2,0,self.screen_width, self.tutorial_text_size * self.tutorial_text.count("\n")))
            #self.draw_text(self.tutorial_text, (self.screen_width//2,0), self.wand_color, self.tutorial_text_size, "NE")
            self.draw_text(self.tutorial_text, (self.screen_width//2,0), self.wand_color2, self.tutorial_text_size, "NE")
    ...
    def draw_circles(self, joint_points):
        ...
        for center in self.circles:
            pygame.draw.circle(self._frame_surface, self.wand_color, (int(center[0]), int(center[1])) , self.circle_radius, self.circle_radius//5) 
        ...
        for i in range(len(self.circles)):
            self.draw_text(text[i], 
                           (self.circles[i][0], self.circles[i][1] - self.circle_radius*2), 
                           self.wand_color, 
                           self.circle_text_size)
    ...
    def draw_circles2(self, joint_points):
        ...
        for center in self.circles2:
            pygame.draw.circle(self._frame_surface, self.wand_color2, (int(center[0]), int(center[1])) , self.circle_radius2, self.circle_radius2//5) 
    ...
    def draw_start_circles(self, joint_points):
        ...
        for center in self.circles:
            pygame.draw.circle(self._frame_surface, self.wand_color, (int(center[0]), int(center[1])) , self.circle_radius, self.circle_radius2//5) 
    ...
    def draw_start_circles2(self, joint_points):
        ...
        customize = (north[0] - self.circle_separation_radius, north[1] + self.circle_separation_radius * 2)
        self.circle_radius2 = abs(int(joint_points[PyKinectV2.JointType_Head].y - joint_points[PyKinectV2.JointType_SpineShoulder].y))//2
        self.circles2 = [start, practice, tutorial, customize]
        for center in self.circles2:
            pygame.draw.circle(self._frame_surface, self.wand_color2, (int(center[0]), int(center[1])) , self.circle_radius2, self.circle_radius2//5) 
        ...
        self.circle_text_size2 =  abs(int(joint_points[PyKinectV2.JointType_Head].y - joint_points[PyKinectV2.JointType_SpineShoulder].y))
        text = ["start",'practice', "tutorial", "customize"]
        for i in range(len(self.circles2)):
            self.draw_text(text[i], (self.circles2[i][0], self.circles2[i][1] - self.circle_radius2*2), self.wand_color2, self.circle_text_size2)

    def draw_customize_circles(self, joint_points):

        self.circle_separation_radius = abs(joint_points[PyKinectV2.JointType_HipLeft].y - joint_points[PyKinectV2.JointType_Head].y)//2
        green = (joint_points[PyKinectV2.JointType_Head].x,joint_points[PyKinectV2.JointType_Head].y) 
        length = (green[0], green[1] + self.circle_separation_radius*5//2)
        blue =  (green[0] + self.circle_separation_radius, green[1] + self.circle_separation_radius)
        red = (blue[0] - self.circle_separation_radius * 2, blue[1])
        self.circle_radius = abs(int(joint_points[PyKinectV2.JointType_Head].y - joint_points[PyKinectV2.JointType_SpineShoulder].y))//2
        self.circles = [red, green, blue]
        for center in self.circles:
            pygame.draw.circle(self._frame_surface, self.wand_color, (int(center[0]), int(center[1])) , self.circle_radius, self.circle_radius//5) 
    ...
    def draw_customize_circles2(self, joint_points):
        #print("Just Like trump, this should not have run")
        self.circle_separation_radius2 = abs(joint_points[PyKinectV2.JointType_HipLeft].y - joint_points[PyKinectV2.JointType_Head].y)//2
        green = (joint_points[PyKinectV2.JointType_Head].x,joint_points[PyKinectV2.JointType_Head].y) 
        length = (green[0], green[1] + self.circle_separation_radius*5//2)
        blue =  (green[0] + self.circle_separation_radius, green[1] + self.circle_separation_radius)
    ...
    def draw_text(self, text, location, color, size, anchor = "center", style = None):
    ...
    def draw_title_screen(self):
        if(self.title_screen):
            ...
            surface_to_draw = pygame.transform.scale(self.title_image, (self._screen.get_width(), target_height))
            self._screen.blit(surface_to_draw, (0,0))
            surface_to_draw = None 

            location = (self.screen_width//4, self.screen_height//4)

            pygame.font.init()
            font = pygame.font.Font(self.title_font, self.title_text_size)
            tWidth, tHeight = font.size(self.title_text)
            toWrite = font.render(self.title_text, True, self.title_color)
            self._screen.blit(toWrite, (location[0] - tWidth//2, location[1]-tHeight//2))
    ...
    def run(self): 
        ...
        while not self._done: 
            ...
            for event in pygame.event.get(): # User did something 
                if event.type == pygame.KEYDOWN:
                    ...
                    #    print(self.trace)
                        ...
                        self.winner_flag = 0
                        self.winner = 2
                    elif(pygame.key.name(event.key) == 'a'): self.avada_kedavra_chance = 1
                    elif(pygame.key.name(event.key) == 'e'): self.avada_kedavra_chance = .05
                    elif(pygame.key.name(event.key) == 'w'):
                        self.win_text_flag = 0  #To test the win screen
            ...
            if self._kinect.has_new_body_frame():  
                ...
                if self._bodies is not None:  
                    ...
                    for i in range(0, self._kinect.max_body_count): 
                        ...
                        if self.body_list[0] == -1 or i == self.body_list[0]:
                            ...
                            if joints[PyKinectV2.JointType_HandRight].TrackingState != PyKinectV2.TrackingState_NotTracked:
                                ...
                                if (joints[PyKinectV2.JointType_Head].TrackingState != PyKinectV2.TrackingState_NotTracked 
                                    ...
                                    if self.customize_mode: #Draw them whether we track it or not, for easier customization
                                        self.draw_customize_circles(joint_points)
                                ...
                                if self.is_wand_tracking:
                                    ...
                                    if(joints[PyKinectV2.JointType_Head].TrackingState != PyKinectV2.TrackingState_NotTracked 
                                        ...
                                        if self.start_screen:
                                            ...
                                            if(self.practice_mode): self.draw_circles(joint_points)
                                        #print(self.circles)
                            ...
                            if joints[PyKinectV2.JointType_HandLeft].TrackingState != PyKinectV2.TrackingState_NotTracked: 
                                ...
                                elif self.start_screen or self.customize_mode or self.main_tutorial or self.practice_mode: pass   #To stop the index out of bounds in the next line
                            ...
                            if(abs(self.left_hand_swipe) > self.left_hand_power_weight): 
                                if self.customize_mode:
                                    if self.left_hand_swipe > 0:
                                        if self.customize_tracker == self.red:
                                            self.wand_color = (self.wand_color[0] + self.wand_color_change_speed, self.wand_color[1], self.wand_color[2])
                                        if self.customize_tracker == self.green:
                                            self.wand_color = (self.wand_color[0], self.wand_color[1] + self.wand_color_change_speed, self.wand_color[2])
                                        if self.customize_tracker == self.blue:
                                            self.wand_color = (self.wand_color[0], self.wand_color[1], self.wand_color[2] + self.wand_color_change_speed)
                                        if self.customize_tracker == self.length:
                                            self.wand_scale += self.wand_scale_change_speed
                                    if self.left_hand_swipe < 0:
                                        if self.customize_tracker == self.red:
                                            self.wand_color = (self.wand_color[0] - self.wand_color_change_speed, self.wand_color[1], self.wand_color[2])
                                        if self.customize_tracker == self.green:
                                        ...
                                        if self.customize_tracker == self.length:
                                            self.wand_scale -= self.wand_scale_change_speed
                                    self.wand_color = list(self.wand_color)
                                    for i in range(len(self.wand_color)):
                                        if self.wand_color[i] >= 255:
                                            self.wand_color[i] = 255
                                        if self.wand_color[i] <= 0:
                                            self.wand_color[i] = 0
                                    if self.wand_scale >= self.max_wand_scale:
                                        self.wand_scale = self.max_wand_scale
                                    if self.wand_scale <= self.min_wand_scale:
                                        self.wand_scale = self.min_wand_scale
                                    self.wand_color = tuple(self.wand_color)
                                elif(self.left_hand_swipe > 0): 
                        ...
                        #I can try to make it robust against multiple people in the color frame
                        #if (self.player_2_flag == 0 and i != self.body_list[0]) or (self.player_2_flag == 1 and i == self.body_list[1]):
                        #    self.body_list[1] = i
                        #    self.player_2_flag = 1
                        if i != self.body_list[0]:
                            ...
                            if joints[PyKinectV2.JointType_HandRight].TrackingState != PyKinectV2.TrackingState_NotTracked:
                                ...
                                if (joints[PyKinectV2.JointType_Head].TrackingState != PyKinectV2.TrackingState_NotTracked 
                                    ...
                                    and joints[PyKinectV2.JointType_SpineShoulder].TrackingState != PyKinectV2.TrackingState_NotTracked):
                                    if self.customize_mode: #Draw them whether we track it or not, for easier customization
                                        self.draw_customize_circles2(joint_points)
                                #To see if the right hand closes on the wand
                                    ...
                                    if (joints[PyKinectV2.JointType_Head].TrackingState != PyKinectV2.TrackingState_NotTracked 
                                        ...
                                        if self.start_screen:
                                            ...
                                            if not self.practice_mode and not self.main_tutorial and not self.customize_mode:   
                                                ...
                                                if(self.do_your_duty_end_flag == 0 and not pygame.mixer.music.get_busy()): 
                                                    self.do_your_duty_end_flag = 1
                                                    #print("Do your duty was stoppe")
                                                self.draw_circles2(joint_points)
                                            if(self.practice_mode): self.draw_circles2(joint_points)
                            ...
                            if(abs(self.left_hand_swipe2) > self.left_hand_power_weight): 
                                if self.customize_mode:
                                    if self.left_hand_swipe2 > 0:
                                        if self.customize_tracker2 == self.red:
                                            self.wand_color2 = (self.wand_color2[0] + self.wand_color_change_speed, self.wand_color2[1], self.wand_color2[2])
                                        if self.customize_tracker2 == self.green:
                                            self.wand_color2 = (self.wand_color2[0], self.wand_color2[1] + self.wand_color_change_speed, self.wand_color2[2])
                                        if self.customize_tracker2 == self.blue:
                                            self.wand_color2 = (self.wand_color2[0], self.wand_color2[1], self.wand_color2[2] + self.wand_color_change_speed)
                                        if self.customize_tracker2 == self.length:
                                            self.wand_scale2 += self.wand_scale_change_speed
                                    if self.left_hand_swipe2 < 0:
                                        if self.customize_tracker2 == self.red:
                                            self.wand_color2 = (self.wand_color2[0] - self.wand_color_change_speed, self.wand_color2[1], self.wand_color2[2])
                                        if self.customize_tracker2 == self.green:
                                        ...
                                        if self.customize_tracker2 == self.length:
                                            self.wand_scale2 -= self.wand_scale_change_speed
                                    self.wand_color2 = list(self.wand_color2)
                                    for i in range(len(self.wand_color2)):
                                        if self.wand_color2[i] >= 255:
                                            self.wand_color2[i] = 255
                                        if self.wand_color2[i] <= 0:
                                            self.wand_color2[i] = 0
                                    if self.wand_scale2 >= self.max_wand_scale:
                                        self.wand_scale2 = self.max_wand_scale
                                    if self.wand_scale2 <= self.min_wand_scale:
                                        self.wand_scale2 = self.min_wand_scale
                                    self.wand_color2 = tuple(self.wand_color2)
                                elif(self.left_hand_swipe2 > 0): 
            ...
            #This is to change the length
            
            if self.customize_mode:
                self.customize_tracker = self.length
            ...
            for centerIndex in range(len(self.circles)):
                ...
                if (self.tip_is_in(center, radius, tip) and (self.trace == [] or self.trace[-1] != centerIndex and not self.spell_clear)):
                    ...
                    if self.customize_mode: 
                        self.customize_tracker = centerIndex
                    else:
                        self.customize_tracker = -1
                ...
                if not self.blocking2 and not self.blocking:
                    ...
                    if self.spell == self.protego:
                        if self.power >= self.protego_power:
                            ...
                            print("Player 1, insufficient power")
                            self.play(self.insufficient_power_sound)
                if self.spell == self.avada_kedavra:
                    if self.power >= self.avada_kedavra_power:
                        chance = random.randint(0,100)/100
                        #print(chance)
                        if chance <= self.avada_kedavra_chance:
                            self.health2 == 0
                            print("Player 1, casts Avada Kedavra")
                            pygame.draw.rect(self._frame_surface, self.wand_color, (0,0,self.screen_width, self.screen_height))
                            pygame.draw.rect(self._frame_surface, self.wand_color2, (0,0,self.screen_width, self.screen_height))
                            pygame.draw.rect(self._frame_surface, self.wand_color2, (0,0,self.screen_width, self.screen_height))
                            self.play_spell(self.avada_kedavra_sound)
                        else: 
                            self.power -= self.avada_kedavra_power
                            self.max_power //= 2
                            self.damage_modifier //= 2
                            self.health //= 2
                            self.play(self.avada_kedavra_sound)
                            self.play(self.avada_kedavra_failed)
                    else:
                        print("Player 1, insufficient power")
                        self.play(self.insufficient_power_sound)
            ...
            if self.customize_mode:
                self.customize_tracker2 = self.length
            ...
            for centerIndex in range(len(self.circles2)):
                ...
                if (self.tip_is_in(center, radius, tip) and (self.trace2 == [] or self.trace2[-1] != centerIndex and not self.spell_clear2)):
                    ...
                    if self.customize_mode: 
                        self.customize_tracker2 = centerIndex
                    else:
                        self.customize_tracker2 = -1
            ...
            if self.spell2 != "" and self.do_your_duty_end_flag:
                if not self.blocking and not self.blocking2:
                    ...
                    if self.spell2 == self.protego:
                        ...
                        self.play(self.insufficient_power_sound)
                if self.spell2 == self.avada_kedavra:
                    ...
                    if chance <= self.avada_kedavra_chance:
                        ...
                        self.power2 -= self.avada_kedavra_power
                        self.max_power2 //= 2
                        self.damage_modifier2 //= 2
                        self.health2 //= 2
                        self.play(self.avada_kedavra_sound)
                        self.play(self.avada_kedavra_failed)
                if(self.blocking2):
